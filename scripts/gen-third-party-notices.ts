...
function renderNonPermissiveNote(deps: ExternalDep[]): string {
  ...
  return `\n${subject} ${named.length === 1 ? 'runs' : 'run'} only as development tooling; their code is not linked into or distributed with any Little Whale artifact.\n`
}
...
Little Whale is licensed under [GPL-3.0-or-later](LICENSE). It depends on the third-party software listed below. Each project remains under its own license; nothing in this file changes those terms.
