...
export function repositoryCommitHash(root: string, environment: NodeJS.ProcessEnv = process.env): string {
  ...
  if (value === undefined) {
    try {
      ...
      // A freshly bootstrapped checkout has no HEAD yet. Keep builds
      // reproducible without requiring a placeholder commit from the user.
      value = '0000000'
    }
  }
