...
During the implementation phase, make the requested code change and explicitly ask the user to validate every changed UI surface visually. Do not inspect the UI visually or run browser, GUI, snapshot, or other visual checks on the user's behalf. Do not run unit tests, TypeScript checks, builds, coverage, documentation, lint, catalog, or broad-gate checks before the user has exercised the change and explicitly confirmed that the visual result is OK. Once that confirmation is received, run the relevant TypeScript checks and build; for a Client or UI change, the build must include the Client artifact that contains the changed surface.
