...
- **Assembled Web snapshots** (`pnpm run test:web`): keyless jsdom snapshots exercise the built Host/Client composition and compare expected outputs under `apps/web/tests/`. `test:web` builds first so snapshots always use the current plugin CSS.
