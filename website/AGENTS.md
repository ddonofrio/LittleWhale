...
Follow the root repository instructions and keep this small site adapter English-only.
...
Keep canonical English prose and generated catalogs in their owning `docs/` tier, then expose selected pages through [docs.ts](docs.ts). Never add locale, route, API, or copied documentation trees under `website/`.
...
Run `pnpm docs:build` after changing this subtree.
