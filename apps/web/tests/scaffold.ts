...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  // real ~/.lw document or credential file.
  ...
  // session is created. Without this a developer's real ~/.lw/skills silently
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.lw/.agent-presets` must not be
    ...
    // the owned temp world so ~/.lw, ~/.agents, and a bundled-root env setting
