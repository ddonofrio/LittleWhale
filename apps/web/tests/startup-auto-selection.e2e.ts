...
describe('web e2e: startup auto-selection', () => {
  ...
  it('keeps the resident Hero and composer nodes when the first Workspace session appears', async () => {
    ...
    const hero = page.locator('[data-hero-chrome]')
    const whale = hero.locator('img[src="/little-whale-logo-text.png"]')
    await whale.waitFor({ timeout: 15_000 })
    expect(await whale.evaluate(node => getComputedStyle(node).height)).toBe('68px')
    await whale.hover()
    expect(await whale.evaluate(node => getComputedStyle(node).animationName)).not.toBe('none')
    await page.evaluate(() => {
      const refs = {
  ...
  it('keeps the hero and the composer on screen while the auto-selected blank session opens', async () => {
    ...
    // hero mark, and a composer that is actually painted (`settling` hides the
    ...
    expect(await page.locator('[data-hero-chrome] img[src="/little-whale-logo-text.png"]').isVisible()).toBe(true)
