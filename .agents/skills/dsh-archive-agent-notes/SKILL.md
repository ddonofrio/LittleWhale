...
description: Use when adding, auditing, pruning, archiving, restoring, or reviewing Agent Notes in Little Whale; checks every new note for superseded active records, classifies implemented notes by future decision value, deletes rejected notes that no longer prevent a tempting fallacy, and applies the frozen archive manifest rules.
---

# Archive Little Whale Agent Notes
...
1. Move the complete `foo.md` note from `implemented/<kind>/` to `archived/<kind>/`; `implemented` is deliberately absent from the archive path.
2. Make no body edits. Insert only `Archived: YYYY-MM-DD` immediately below `Status: implemented`, using the archival date.
3. Update the append-only archive manifest for the metadata-only edit. Do not reformat, update facts, or repair links inside the note.
