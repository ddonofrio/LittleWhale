...
Composer chrome copy (pager, buttons, placeholders, validation feedback) comes from the `question` namespace of `dsh-client-locale`; the plugin hands the entry its bound translator plus the locale snapshot source through the inject face, so a locale update re-renders a mounted composer. Question and option text arrives from the model and renders verbatim; carrier failure messages also display untranslated.
