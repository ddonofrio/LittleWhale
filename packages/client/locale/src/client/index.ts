/**
 * Browser-side locale registry. Little Whale intentionally exposes one visible
 * locale: English. Upstream dictionaries remain available for mirror parity,
 * but no non-English locale is selectable or loaded into the product UI.
 */
...
import { LOCALE_SETTINGS_NAMESPACE, type LocaleId, type LocaleSettings } from '../locale-settings.ts'
...
const LOCALES: readonly LocaleDefinition[] = Object.freeze([{ id: 'en', label: 'English' }])
...
export class LocaleRuntime {
  ...
  constructor(ctx: Context, host?: SettingsScope<LocaleSettings>) {
    ...
    if (host !== undefined) {
      ctx.effect(() => host.subscribe(() => { this.adopt() }), 'locale: settings scope adoption')
      this.adopt()
    }
  }
  ...
  setLocale(id: string): void {
    if (id !== 'en') throw new Error(`locale "${id}" is not available in Little Whale`)
    if (this.snapshot.active !== 'en') this.publish('en', true)
  }
...
/**
 * Client plugin body: provide the locale service with the English-visible
 * dictionary set. The upstream locale settings namespace stays dormant for
 * compatibility, but there is no language selector in Little Whale.
...
export function apply(ctx: ClientContext): void {
  ...
  const sync = (snapshot: LocaleSnapshot): void => { syncDocumentLanguage(snapshot.active) }
