...
`resolveDshHome()` resolves the single-root Little Whale home. Precedence, highest first: an explicit configured path, `$DSH_HOME`, then `~/.lw`. The harness keeps all user data under one root.
...
`dshHomeDisplay()` names an active root symbolically for user-facing paths: `~/.lw` for the default home, `$DSH_HOME` for any configured home. It never leaks an absolute machine path.

`DSH_HOME_DIR_NAME` owns the default user-data directory name: `.lw`.

`defaultDshHome()` returns the default Little Whale home by joining the operating-system home directory with `.lw`, using Node's platform path rules.
