...
Both modes use bounded exponential backoff with symmetric jitter. A valid `providerRetryAfterMs` replaces local backoff without jitter, even when it exceeds `maxDelayMs`; the runtime timer safety limit remains enforced. The local cap applies to delays calculated by this plugin, not to an explicit provider instruction.
