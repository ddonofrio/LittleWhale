...
- **User threshold policy** — when `ctx.compactionPolicy` is installed, its global 75% default or exact provider/model override controls the automatic threshold as a fraction of the complete context window. The request output reserve remains a hard ceiling, so compaction may start earlier when the configured output budget leaves less input capacity. If a user threshold is below the inherited recent-tail ratio, the inherited ratio is reduced to 80% of that threshold so the selected position remains usable; explicit absolute retention budgets keep their validation rules.
