...
describe('agent-loop settings section', () => {
  ...
  it('never offers the composed agents array to the settings document', async () => {
    ...
    expect(Object.keys(descriptor?.value as object)).toEqual([
      ...
      'loopDetectionDetectOnText',
      'loopDetectionDetectOnReasoning',
  ...
  it('refuses settings that disable every detection stream', async () => {
    const bench = await boot()

    await expect(bench.ctx.settings.update(AGENT_LOOP_SETTINGS_NAMESPACE, {
      loopDetectionDetectOnText: false,
      loopDetectionDetectOnReasoning: false,
      loopDetectionDetectOnToolCall: false,
    })).rejects.toThrow('agent loopDetection requires at least one detected stream')
