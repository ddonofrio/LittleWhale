...
interface Config {
  ...
  agents: Array<{
    ...
    loopDetection?: {
      ...
      detectOnText?: boolean    // default true
      detectOnReasoning?: boolean // default true
      detectOnToolCall?: boolean // default false
      includeLoop?: boolean    // default true
      minTokens?: number       // default 16
      maxToolCallDetections?: number // default 32; consecutive tool-call loop detections before failure
      ...
      toolCallFirstPrompt?: string  // default repeated-token restart prompt
      toolCallSecondPrompt?: string // default repeated-token warning and restart prompt
      toolCallThirdPrompt?: string  // default stop-and-report prompt
      compactBeforeFailing?: boolean // default true; compact before the fourth detection fails
    }
  }>
}
```

Configured agents start automatically. A model call requires both `provider` and `model`; `agent/request` may supply a missing pair before dispatch. An optional positive `maxTokens` seeds each conversation request's output cap and is logged in its request header. `loopDetection` watches token-like word/punctuation units from the selected text, reasoning, and tool-call streams in each response; `detectOnText` and `detectOnReasoning` default to `true`; `detectOnToolCall` defaults to `false`, and at least one must remain enabled. When the same suffix appears three times, the loop stops at once, optionally records the partial assistant response, and queues the configured intervention prompt for the next step. Adapters that emit complete blocks without deltas are covered through `block-end`, without counting a block twice. Tool-call detections use a separate consecutive counter capped by `maxToolCallDetections`, which defaults to `32`; text and reasoning detections retain the four-detection terminal limit. The first, second, and third consecutive detections use their corresponding prompts; reaching the applicable limit raises `LLM_LOOP` with `LLM in infinite loop.`. A normally completed response resets both counters. The default text/reasoning prompts are `<continue>`, `THIS IS A SYSTEM MESSAGE: <You are in an LLM loop; please output the response now>`, and `THIS IS A SYSTEM MESSAGE: <Please stop. Explain to the user, in detail, the current status, what you have done, and what is missing; do not continue with your task>`. The default tool-call prompts are `You have repeated {maximum-tool-call-detections} tokens. This might be not an issue but a restriction. Please restart the tool call in another way.`, `Ops. Again! WARNING, this is the second consecutive time that you are repeating the last {maximum-tool-call-detections} tokens. This might be not an issue but a restriction. Please restart the tool call in another way.`, and `THIS IS A SYSTEM MESSAGE: <Please stop. Explain to the user, in detail, the current status, what you have done, and what is missing; do not continue with your task>`. The `{maximum-tool-call-detections}` placeholder is replaced with the configured maximum before the prompt is sent. The Models > Behavior settings surface exposes the same policy as `agent-loop` fields (`loopDetectionEnabled`, `loopDetectionDetectOnText`, `loopDetectionDetectOnReasoning`, `loopDetectionDetectOnToolCall`, `loopDetectionIncludeLoop`, `loopDetectionMinTokens`, `loopDetectionMaxToolCallDetections`, and the three prompt fields); those values apply to new agents unless an explicit `AgentOptions.loopDetection` field overrides them. `maxParallelToolCalls` bounds every agent's rolling pool for parallel-safe calls and defaults to `10`. `agents` is deliberately absent from the Settings section — it is consumed once when the service starts, so a stored change could only look like it had an effect. `cwd` applies only to fresh sessions, while `resumeSessionId` retains persisted metadata. Configured agents use the deployment persona, and programmatic setup can shadow it per agent. This plugin supplies the per-agent `provider`, `model`, and `cwd` prompt variables; harness identity and deployment persona belong to `dsh-system-prompt`.
...
The Models > Behavior settings surface exposes the same policy through `loopDetectionMaxToolCallDetections`, the three `loopDetectionToolCall*Prompt` fields, and `loopDetectionCompactBeforeFailing` in addition to the existing loop-detection fields. Text and reasoning use the general recovery prompts; tool-call detections use their dedicated recovery prompts.
