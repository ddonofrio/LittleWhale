...
/** Stream categories that may contribute to repeated-output detection. */
export interface LoopDetectionStreamSelection {
  text: boolean
  reasoning: boolean
  toolCall: boolean
}
 ...
 * @param selection - stream categories to include in the detector.
 * @returns content to append to the detector input, or `undefined` for metadata.
 */
export function loopTextForChunk(
  chunk: StreamChunk,
  deltaIndexes: Set<number>,
  selection: LoopDetectionStreamSelection,
): string | undefined {
  switch (chunk.type) {
    case 'text-delta':
      deltaIndexes.add(chunk.index)
      return selection.text ? chunk.text : undefined
    case 'reasoning-delta':
      deltaIndexes.add(chunk.index)
      return selection.reasoning ? chunk.text : undefined
    case 'tool-call-delta':
      deltaIndexes.add(chunk.index)
      return selection.toolCall ? `${chunk.name ?? ''}${chunk.argumentsDelta}` : undefined
    case 'block-end':
      if (deltaIndexes.has(chunk.index)) return undefined
      return contentBlockLoopText(chunk.block, selection)
    default:
      return undefined
  }
}
...
function contentBlockLoopText(block: ContentBlock, selection: LoopDetectionStreamSelection): string | undefined {
  switch (block.type) {
    case 'text':
      return selection.text ? block.text : undefined
    case 'reasoning':
      return selection.reasoning ? block.text : undefined
    case 'tool-call':
      return selection.toolCall ? `${block.name}${block.arguments}` : undefined
    default:
      return undefined
  }
}
