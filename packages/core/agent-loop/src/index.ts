...
function loopDetectionFromSettings(settings: AgentLoopSettings): Required<LoopDetectionOptions> {
  return {
    ...
    detectOnText: settings.loopDetectionDetectOnText,
    detectOnReasoning: settings.loopDetectionDetectOnReasoning,
...
function resolveAgentOptions(options: AgentOptions, settings: AgentLoopSettings): AgentOptions {
  ...
  const usesDefaultPolicy = configured.enabled === DEFAULT_LOOP_DETECTION.enabled
    && configured.detectOnText === DEFAULT_LOOP_DETECTION.detectOnText
    && configured.detectOnReasoning === DEFAULT_LOOP_DETECTION.detectOnReasoning
...
function assertAgentOptions(options: AgentOptions): void {
  ...
  if (loopDetection.detectOnText === false
    && loopDetection.detectOnReasoning === false
    && loopDetection.detectOnToolCall === false) {
    throw new TypeError('agent loopDetection requires at least one detected stream')
  }
...
export interface AgentLoopSettings {
  ...
  /** Whether text output contributes to repeated-output detection. */
  loopDetectionDetectOnText: boolean
  /** Whether reasoning output contributes to repeated-output detection. */
  loopDetectionDetectOnReasoning: boolean
  /** Whether tool-call output contributes to repeated-output detection. */
...
export const AGENT_LOOP_SETTINGS_SCHEMA: z<AgentLoopSettings> = z.object({
  ...
  loopDetectionDetectOnText: z.boolean().default(DEFAULT_LOOP_DETECTION.detectOnText),
  loopDetectionDetectOnReasoning: z.boolean().default(DEFAULT_LOOP_DETECTION.detectOnReasoning),
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const entry: AgentLoopSettings = {
      ...
      loopDetectionDetectOnText: DEFAULT_LOOP_DETECTION.detectOnText,
      loopDetectionDetectOnReasoning: DEFAULT_LOOP_DETECTION.detectOnReasoning,
    ...
    installSettingsSection(ctx, AGENT_LOOP_SETTINGS_NAMESPACE, AGENT_LOOP_SETTINGS_SCHEMA, entry, {
      ...
      validate: (value) => {
        ...
        if (!value.loopDetectionDetectOnText
          && !value.loopDetectionDetectOnReasoning
          && !value.loopDetectionDetectOnToolCall) {
          throw new TypeError('agent loopDetection requires at least one detected stream')
        }
      },
      setSource: (current) => {
