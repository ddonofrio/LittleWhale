...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@ddonofrio/littlewhale'
...
const IDENTITY = 'You are an AI agent powered by Little Whale.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are Little Whale.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are Little Whale.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
