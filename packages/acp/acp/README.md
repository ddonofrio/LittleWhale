...
`pnpm --dir /path/to/LittleWhale run demo:acp` boots the repository's automation server composition. A parent Little Whale can spawn it through [`@deepseek-ai/dsh-subagent-acp`](../../subagent/subagent-acp/README.md); other ACP clients need only the core methods above.
